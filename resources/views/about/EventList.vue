  ...
  <section class="history_time">
    <div class="container">
      ...
      <div class="time_block">
        <div class="time_box" v-for="year in Object.keys(events).reverse()" :key="year">
          <h2 class="year">{{ year }}</h2>
          <ul class="event_block" v-for="event in events[year]" :key="event.id">
            <li class="event_box">
              <router-link :to="`/about/event/${event.id}`" :title="event.title">
        ...
        <infinite-loading @infinite="infiniteLoadingHandler" />
      </div>
    </div>
  </section>
</template>
